<template>
  <v-app>
    <v-toolbar elevation="0" style="flex: initial; width: 100%">
      <!--로고-->
      <v-toolbar-title>
        <nuxt-link to="/">
          <v-avatar height="80px" width="200px" tile>
            <img src="../static/logo3.png" />
          </v-avatar>
        </nuxt-link>
      </v-toolbar-title>
      <!--툴바 목록-->
      <v-toolbar-items>
        <v-menu open-on-hover offset-y>
          <template #activator="{ on, attrs }">
            <v-btn depressed v-bind="attrs" v-on="on"> 텍스트 번역 </v-btn>
          </template>
          <v-list dense>
            <v-list-item-group>
              <v-list-item>
                <v-list-item-action-text>소개</v-list-item-action-text>
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text @click="go('/user/textmain')"
                  >텍스트 번역 의뢰</v-list-item-action-text
                >
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text>번역 단가</v-list-item-action-text>
              </v-list-item>
            </v-list-item-group>
          </v-list>
        </v-menu>
        <v-menu open-on-hover offset-y>
          <template #activator="{ on, attrs }">
            <v-btn depressed v-bind="attrs" v-on="on"> 영상 번역 </v-btn>
          </template>
          <v-list dense>
            <v-list-item-group>
              <v-list-item>
                <v-list-item-action-text>소개</v-list-item-action-text>
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text
                  >영상 번역 의뢰</v-list-item-action-text
                >
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text>번역 단가</v-list-item-action-text>
              </v-list-item>
            </v-list-item-group>
          </v-list>
        </v-menu>
        <v-menu open-on-hover offset-y>
          <template #activator="{ on, attrs }">
            <v-btn depressed v-bind="attrs" v-on="on"> 음성 번역 </v-btn>
          </template>
          <v-list dense>
            <v-list-item-group>
              <v-list-item>
                <v-list-item-action-text>소개</v-list-item-action-text>
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text
                  >음성 번역 의뢰</v-list-item-action-text
                >
              </v-list-item>
              <v-list-item>
                <v-list-item-action-text>번역 단가</v-list-item-action-text>
              </v-list-item>
            </v-list-item-group>
          </v-list>
        </v-menu>
      </v-toolbar-items>

      <v-spacer />
      <!--페이지 언어 선택-->
      <v-toolbar-items>
        <v-menu offset-y>
          <template #activator="{ on, attrs }">
            <v-btn tile icon v-bind="attrs" v-on="on">
              <v-icon>mdi-translate</v-icon>
            </v-btn>
          </template>
          <v-list>
            <v-list-item-group v-model="language" mandatory>
              <v-list-item value="한국어">
                <v-list-item-title class="text-center">
                  한국어
                </v-list-item-title>
              </v-list-item>
              <v-list-item value="영어">
                <v-list-item-title class="text-center">
                  English
                </v-list-item-title>
              </v-list-item>
            </v-list-item-group>
          </v-list>
        </v-menu>
      </v-toolbar-items>
    </v-toolbar>
    <nuxt />
    <snack-bar />
  </v-app>
</template>

<style scoped>
</style>

<script>
import SnackBar from "../components/SnackBar.vue";
export default {
  components: { SnackBar },
  data() {
    return {
      language: "한국어",
    };
  },
  watch: {
    language() {
      this.$manage.setLanguage({ language: this.language });
    },
  },
  methods: {
    go(url) {
      this.$router.push({ path: `${url}` });
    },
  },
};
</script>
